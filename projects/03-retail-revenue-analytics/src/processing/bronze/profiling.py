...
def select_main_csv_file(csv_files: list[Path]) -> Path:
    ...
    ties. This is a raw-stage profiling rule only. It is not a final business
    grain decision for future fact or dimension modeling.
    """
    if not csv_files:
...
def profile_csv_file(csv_path: Path) -> dict[str, Any]:
    ...
    Pandas is used as the local profiling engine for this foundation step.
    """
