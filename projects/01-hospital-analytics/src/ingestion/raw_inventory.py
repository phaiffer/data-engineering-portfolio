...
def get_raw_data_dir() -> Path:
    """Return the local raw Bronze directory for the hospital patient flow dataset."""
...
def is_real_data_file(path: Path) -> bool:
    """
    Return whether a file should be treated as input data for Bronze profiling.

    KaggleHub may create marker files under hidden folders such as `.complete`.
    Those files are useful operational artifacts, but they are not source data.
    """
    try:
...
def list_supported_data_files(raw_data_dir: Path | None = None) -> list[Path]:
    """List supported source data files that can be processed by the Bronze profiler."""
