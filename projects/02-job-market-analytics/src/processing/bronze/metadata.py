...
BRONZE_SCOPE_NOTES = [
    "Initial Bronze phase only: raw file inventory, source metadata capture, and CSV profiling.",
    "No Silver standardization or Gold analytical modeling is applied in this step.",
    "Raw column names, values, duplicate rows, and nulls are preserved.",
    "Pandas is used for local profiling; Spark is not forced in this environment.",
]

SOURCE_NOTES = [
    "Dataset is sourced from Kaggle through kagglehub.",
    "The current analytical domain is job market and labor analytics.",
    "Business semantics are intentionally deferred until Silver and Gold contracts are designed from observed source columns.",
]
...
def write_bronze_metadata(
    ...
    filename: str = "ai_powered_job_market_insights_bronze_metadata.json",
) -> Path:
