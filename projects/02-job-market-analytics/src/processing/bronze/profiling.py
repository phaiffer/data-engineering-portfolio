...
def select_main_csv_file(csv_files: list[Path]) -> Path:
    ...
    Rule: choose the largest CSV by file size, then by path for deterministic
    ties. This is explainable at the raw inventory stage and can be replaced by
    a documented source-specific rule in Silver when the analytical grain is
...
def profile_csv_file(csv_path: Path) -> dict[str, Any]:
    """
    Profile a raw CSV file without changing columns, values, or records.

    Pandas is used as the local profiling engine to keep this foundation step
    realistic for the current environment.
    """
